...
class Ball(pygame.sprite.Sprite):
    ...
    def __init__(self, radius, speed, rng,
        ...
        # initialize ball velocity at random angle
        angle = -2*math.pi + self.rng.random_sample()*4*math.pi
        # speed should be cos
        speed_x = speed * math.cos(angle)
        speed_y = speed * math.sin(angle)
        # speed = self.rng.random_sample() * 2.0 - 1.0
        # self.vel = vec2d((speed, -1.0 * speed))
        self.vel = vec2d((speed_x, speed_y))
...
class Pong(PyGameWrapper):
    ...
    def getGameState(self):
        ...
        """
        state = {
            "player_x": self.agentPlayer.pos.x,
            "player_y": self.agentPlayer.pos.y,
            "padde_width": self.paddle_width,
